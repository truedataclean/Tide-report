...
def save_to_word(file_info, grouped_data, output_path, linz_logo_path):
    ...
    coordinates = f"Lat. {(file_info[2]).replace('Â', '')} Long. {(file_info[3]).replace('Â', '')}"
            ...
            for idx, row_date in enumerate(range(1, 9)):  # Changed range to 1 to 9 for 8 days
                if date == str(row_date):
                    ...
                    if int(month) in [1, 2, 3, 10, 11, 12]:
                        ...
                        # Check if the first time is between 1:59 and 3:00 and print it
                        # if times and times[0]:
                        #     try:
                        #         hour, minute = map(int, times[0].split(':'))
                        #         if (hour == 1 and minute >= 59) or (hour == 2) or (hour == 3 and minute == 0):
                        #             print(f"First row between 1:59 and 3:00: {times[0]}")
                        #     except Exception as e:
                    ...
                    if int(month) in [1, 2, 3, 10, 11, 12]:
                        ...
                        # Check if the first time is between 1:59 and 3:00 and change its style
                        # if times and times[0]:
                        #     try:
                        #         hour, minute = map(int, times[0].split(':'))
                        #         if (hour == 1 and minute >= 59) or (hour == 2) or (hour == 3 and minute == 0):
                        #             # Change style: make it italic and red
                        #             run = target_row[2].paragraphs[0].runs[0]
                        ...
                        if ontimes:
                            ...
                            for remaining_time in remaining_times:
                                if remaining_time:
                                    ...
                                    bold_paragraph.runs[-1].font.size = Pt(9)  # Set font size for the remaining times
