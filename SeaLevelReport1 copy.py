...
def add_caution(doc):
    ...
    caution_paragraph.paragraph_format.space_after = Pt(5)  # Reduce spacing after the title
...
def add_daylight(doc):
    ...
    daylight_paragraph.paragraph_format.space_after = Pt(5)  # Reduce spacing after the title
        ...
        for row in rows:
                ...
                if date == str(row_date):
                    ...
                    if int(month) in [1, 2, 3, 10, 11, 12]:
                        ...
                        if remaining_times:
                            ...
                            non_bold_paragraph.runs[0].font.size = Pt(10)
                            non_bold_paragraph.paragraph_format.space_after = Pt(0)  # Reduce spacing

        

                    target_row[2].text = "\n".join([rest[i] for i in range(1, len(rest), 2)])
                    target_row[2].paragraphs[0].runs[0].font.size = Pt(10)
                    target_row[2].paragraphs[0].paragraph_format.space_after = Pt(0)  # Reduce spacing
            ...
            for idx, row_date in enumerate(range(9, 17)):  # Changed range to 9 to 16 for the next 8 days
                if date == str(row_date):
                    ...
                    target_row[5].paragraphs[0].paragraph_format.space_after = Pt(0)  # Reduce spacing
                    if int(month) in [1, 2, 3, 10, 11, 12]:
            ...
            for idx, row_date in enumerate(range(17, 25)):  # Changed range to 17 to 24 for the next 8 days
                if date == str(row_date):
                    ...
                    target_row[8].paragraphs[0].paragraph_format.space_after = Pt(0)  # Reduce spacing
                    if int(month) in [1, 2, 3, 10, 11, 12]:
            ...
            for idx, row_date in enumerate(range(25, 32)):  # Changed range to 25 to 31 for the next 8 days
                if date == str(row_date):
                    ...
                    target_row[11].paragraphs[0].paragraph_format.space_after = Pt(0)  # Reduce spacing
...
def main():
    ...
    output_path = r'C:\Projects\Glen\SeaLevelReport.docx'
    
    # Read the CSV file
    file_info, header, data = read_csv(file_path)
    
    # Extract region name and coordinates
    region_name = file_info[1]
    coordinates = f"Lat. {file_info[2]} Long. {file_info[3]}"
    
    # Print extracted information (or further processing can be added here)
    print(f"Region Name: {region_name}")
    print(f"Coordinates: {coordinates}")
    
    # Group data by month and print
    grouped_data = group_data_by_month(data)
    # for month, rows in grouped_data.items():
    #     if month == '1':  # Check if the month is 1
    #         print(f"Month: {month}, Data: {rows}")
